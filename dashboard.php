...
if (isset($_SESSION["user_role"]) && $_SESSION["user_role"] == 'root_admin' or isset($_SESSION["user_role"]) && $_SESSION["user_role"] == 'general') {
    ...
    $deposit_total = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(SUM(amount), 0) as total FROM deposit"))['total'];
    $costing_total = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(SUM(costing_amount), 0) as total FROM costing"))['total'];
    ...
    $office_cost_total = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(SUM(office_amount), 0) as total FROM office_cost"))['total'];
                    ...
                    <div class="content-wrapper">
                        ...
                        <div class="page-header d-flex justify-content-between align-items-center mb-4">
                            <div>
                                ...
                                <p class="text-white mb-0">Welcome back! Here's your project overview.</p>
                            </div>
                                ...
                                <div class="card chart-card">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-center mb-4">
                                            <div>
                                                <h4 class="card-title mb-1">Financial Overview</h4>
                                                <p class="text-muted mb-0">Income, expenses, and balance summary</p>
                                            </div>
                                            <div class="dropdown">
                                            ...
                                            <?php while ($tp = mysqli_fetch_assoc($top_projects)): ?>
                                                <div class="top-project-item">
                                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                                        <span
                                                            class="font-weight-bold"><?= htmlspecialchars($tp['project_name']) ?></span>
                                                        <span class="text-primary font-weight-bold">৳
                                                            <?= number_format($tp['total_deposit']) ?></span>
                                                    </div>
                                                    <div class="progress" style="height: 6px;">
                                                        <div class="progress-bar bg-primary"
                                                            style="width: <?= min(100, ($tp['total_deposit'] / max($deposit_total, 1)) * 100) ?>%">
                                                        </div>
                                                    </div>
                                                </div>
                                            <?php endwhile; ?>
                                                ...
                                                <tbody>
                                                    <?php while ($rp = mysqli_fetch_assoc($recent_projects)): ?>
                                                        <tr>
                                                            ...
                                                            <td><span
                                                                    class="badge badge-info"><?= date('M d, Y', strtotime($rp['starting_date'])) ?></span>
                                                            </td>
                                                        </tr>
                                                    <?php endwhile; ?>
                                                </tbody>
                                            ...
                                            <table class="table table-hover">
                                                ...
                                                <tbody>
                                                    <?php while ($rd = mysqli_fetch_assoc($recent_deposits)): ?>
                                                        <tr>
                                                            <td class="font-weight-bold text-muted">
                                                                <?= htmlspecialchars($rd['deposit_name']) ?></td>
                                                            <td class="text-muted"><?= htmlspecialchars($rd['project_name']) ?>
                                                            </td>
                                                            <td class="text-success font-weight-bold">+৳
                                                                <?= number_format($rd['amount']) ?></td>
                                                            <td><span
                                                                    class="badge badge-success"><?= date('M d', strtotime($rd['deposit_date'])) ?></span>
                                                            </td>
                                                        </tr>
                                                    <?php endwhile; ?>
                                                </tbody>
