							...
							<table class="table table-bordered">
								...
								<tbody>
									<?php 
										include_once 'db/db.php';
											...
											if(isset($_GET["project_delete"])){
												$get_project_id = $_GET["project_delete"];
												$delete_project_row = "DELETE FROM project WHERE id=$get_project_id";
												$delete_project_query = mysqli_query($conn, $delete_project_row);
												if($delete_project_query==true){
													echo "<span class='text-info'>Project Deleted Successfully</span>";
												}else{
													echo "<span class='text-info'>You have To remove child of Project</span>";
												}
											}
											// Edit Data Update

											if(isset($_POST["project_update_submit"])){
												$update_sql ="UPDATE project SET project_name='".$_POST['project_name']."', address='".$_POST['project_address']."', starting_date='".$_POST['project_upddate_date']."' WHERE id='".$_POST['project_id']."'";
									...
									<tr>
										...
										<td><a class="text-decoration-none" style="color:#0094BC;" href="vandor_view.php?project_id=<?php echo $row['id']; ?>"> <?php echo $row["project_name"]; ?> </a></td>
										<td> <span style="color:#0094BC;"> <?php echo $row["address"]; ?> </span></td>
										<td> <span style="color:#0094BC;"> <?php echo $row["oroject_date"]; ?></span> </td>
										<td><a class="text-decoration-none text-success" href="project_view.php?project_edit_id=<?php echo $row['id']; ?>"> Edit </a></td>
									...
									<script type="text/javascript">
										function checkalert(id){
											...
											if(msg){
												document.location.href="project_view.php?project_delete="+id;
											}
										}
									</script>
									<?php }
